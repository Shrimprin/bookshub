...
- **Background Service Worker** (`handleExternalMessage`)
  - `sender.origin` と message type を Zod 検証 (`isAllowedOrigin` → `externalExtensionMessageSchema`)
  - `_triggerScrapeInProgress` で同時押し race を mutex 防止
  - 既存 flag があれば「TTL 内 + `chrome.tabs.get` でタブ生存」のとき `ALREADY_IN_PROGRESS` で拒否、孤児なら `clearKindleScrapeTrigger()` で回収
  - 新規 trigger 受付時の手順 (実装順):
    1. `STORE_REGISTRY[store]` から trigger URL を取得 (`?pageNumber=1` 含む)
    2. `chrome.tabs.create({ url, active: false })` で背景タブを開く (返り値で `tab.id` 取得)
    3. `setKindleScrapeTrigger({ tabId, startedAt, source: 'web', store })` で session storage に flag を書く
    4. `{ success: true }` を返す
    - 上記 try/catch で囲み、throw 時は best-effort で `chrome.tabs.remove(createdTabId)` + `clearKindleScrapeTrigger()` を実行し `TAB_CREATE_FAILED` を返す
  - `chrome.storage.session.setAccessLevel({ accessLevel: 'TRUSTED_AND_UNTRUSTED_CONTEXTS' })` は SW 起動時のトップレベルで一度だけ実行する (trigger フローに含まれない、idempotent)
...
1. Web 本棚の「Kindle から取り込み」ボタン押下 → `TRIGGER_SCRAPE` 外部メッセージ送信
2. Background が `chrome.tabs.create()` で `?pageNumber=1` から始まる Kindle ページを背景タブで開く (`active: false`)
3. 続けて `setKindleScrapeTrigger({ tabId, ... })` で `chrome.storage.session` に flag を書込 (順序: tab 作成 → flag 書込)
4. Content Script (kindle.ts) が document_idle で起動 → `getKindleScrapeTrigger()` で flag 存在 + TTL 内を確認
5. さらに `IS_TRIGGER_TAB` RPC で background に `sender.tab.id === trigger.tabId` を問い合わせ、自タブが trigger 本体タブであることを確認
6. flag が無い / TTL 超過 / 別タブのいずれかなら early return (手動訪問判定 / 別タブの偶発訪問)
7. 以降のページネーション処理は従来通り
...
- `TRIGGER_TTL_MS = 10 分` (`apps/extension/src/utils/constants.ts` で集約):trigger flag の生存期間上限
- Content Script: ページ遷移後の再起動時に TTL チェック。超過していたら `notifyAbort('UNEXPECTED_ERROR')` を送って Background 側 cleanup に任せる
- Background `triggerScrape`: 新規 trigger 受付時に既存 flag を確認し、`chrome.tabs.get(tabId)` で実体タブの生存を verify。タブ消滅または TTL 超過は孤児として `clearKindleScrapeTrigger()` で回収後に再受付
- Background `chrome.tabs.onRemoved`: 閉じられた tabId が `trigger.tabId` と一致する時のみ cleanup (TTL は判定に使わない)
- `cleanupAndRecordResult` 内では **flag clear → tabs.remove** の順で実行する (逆だと自分で閉じたタブの onRemoved が成功結果を上書きするレースが発生する)
