...
| プライマリ (CTA)       | `--primary` / `--primary-foreground`         | `325 85% 52%` / `0 0% 100%`   |
| セカンダリ (Kindle 等) | `--secondary` / `--secondary-foreground`     | `195 90% 42%` / `0 0% 100%`   |
| アクセント             | `--accent` / `--accent-foreground`           | `280 70% 50%` / `0 0% 100%`   |
| ミュート               | `--muted` / `--muted-foreground`             | `280 20% 95%` / `260 10% 45%` |
| 破壊的操作             | `--destructive` / `--destructive-foreground` | `0 75% 50%` / `0 0% 98%`      |
| ボーダー / 入力枠      | `--border` / `--input`                       | `280 25% 88%`                 |
| フォーカスリング       | `--ring`                                     | `325 85% 55%`                 |
...
| プライマリ (ネオンマゼンタ) | `--primary` / `--primary-foreground`         | `320 100% 60%` / `240 15% 6%` |
| セカンダリ (シアン)         | `--secondary` / `--secondary-foreground`     | `190 100% 55%` / `240 15% 6%` |
| アクセント (バイオレット)   | `--accent` / `--accent-foreground`           | `280 80% 55%` / `300 20% 96%` |
| ミュート                    | `--muted` / `--muted-foreground`             | `260 15% 14%` / `280 15% 65%` |
| 破壊的操作                  | `--destructive` / `--destructive-foreground` | `0 90% 60%` / `0 0% 98%`      |
| ボーダー / 入力枠           | `--border` / `--input`                       | `280 40% 22%`                 |
| フォーカスリング            | `--ring`                                     | `320 100% 65%`                |
...
| ユーティリティ          | 変数                      | 用途                                           |
| ----------------------- | ------------------------- | ---------------------------------------------- |
| `shadow-glow-primary`   | `--shadow-glow-primary`   | プライマリ CTA / カードホバー / 入力フォーカス |
| `shadow-glow-secondary` | `--shadow-glow-secondary` | シアン系バッジ (Kindle, 巻数バッジ)            |
| `shadow-glow-soft`      | `--shadow-glow-soft`      | LP のヒーローバッジ / カード控えめ強調         |
...
| Button         | `secondary`                              | シアン基調 (ホバーで `shadow-glow-soft`)     |
