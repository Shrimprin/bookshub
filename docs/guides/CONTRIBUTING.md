...
`/api/books` POST (新規シリーズ登録時の次巻 sync lookup) と Cron Trigger (5 分おきの次巻 refresh) は `SUPABASE_SERVICE_ROLE_KEY` を必要とする。これも Cloudflare Worker secret として登録する (RLS で UPDATE 全 deny の `series` テーブルをサーバ側からのみ書き込むため)。
...
# 次巻ステータス書き込み + cron 用 (Issue #15)。設定しないと bookshelf に次巻バッジが
# 表示されず、cron 起動時に 500 エラーになる。
echo -n "<SUPABASE_SERVICE_ROLE_KEY 値>" | pnpm exec wrangler secret put SUPABASE_SERVICE_ROLE_KEY --name bookhub-web
```

両方未設定の場合 `/api/books/search` は `source: "none"` を返す (機能縮退)。`SUPABASE_SERVICE_ROLE_KEY` 未設定の場合は次巻バッジ機能のみ無効化されるが、本体登録機能は影響を受けない (sync lookup は best-effort)。Secret rotation は同コマンドで上書き可能。値の確認は `wrangler secret list --name bookhub-web` (値そのものは表示されない)。

将来 secret 自動同期を CD ワークフローに組み込む場合は別 issue で対応する。

##### Cron Trigger (Issue #15 次巻ステータス refresh)

`wrangler.jsonc` で `triggers.crons: ["*/5 * * * *"]` を有効化済み。デプロイ時に Cloudflare 側で自動的に schedule 登録される。**Workers 無料プランでも Cron Triggers は利用可能** (1 起動 10ms CPU 制限内に収まるよう batch=3, sleep=1.1s で設計)。

Cron 動作確認:

```bash
# 直近の cron 実行ログを観測 (Cloudflare dashboard の Workers > bookhub-web > Logs でも可)
pnpm exec wrangler tail --name bookhub-web --format pretty
```

`[cron/refresh-next-volumes]` の log line に `processed`, `errors`, `durationMs` が出る。`durationMs` が CPU time ではなく wall time (fetch I/O 含む) である点に注意。CPU time が 10ms を超えると Cloudflare が worker を kill するので、必要なら `wrangler.jsonc` の cron 設定を `batch=1` に下げる、または cron 周期を伸ばす。
